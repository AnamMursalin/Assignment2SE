...
public:
    ...
    vector<string> getSpeciesNames() const {
        return {
            "Warrior","Builder","Gatherer","Soldier", "Drone", "Protector","Researcher","Queen","Scout", "Hunter"
        };
    }
};
...
void executecommand(const string& actiontoperform, Meadow* meadow)
 {
    istringstream stream(actiontoperform);
    string action;
    stream >> action;
...
int main()
{
    ...
    int rollNumber = 6;  // Predefined roll number
    ...
    {
        displayMenu();
        cout << "Enter a actiontoperform:";
        getline(cin, actiontoperform);
        executecommand(actiontoperform, meadow);
    } while (actiontoperform != "quit");
